# Found by AI

**From Invisible to AI's #1 Recommendation**
Make your business the go-to choice in ChatGPT, Gemini, Perplexity, Grok, and emerging AI agents — before users even search traditionally.

[![Live Site](https://img.shields.io/badge/Live%20Site-discovered--by--ai.com-brightgreen)](https://discovered-by-ai.com)
[![X Follow](https://img.shields.io/badge/Follow%20@OmriHochman-blue?logo=x&logoColor=white)](https://x.com/OmriHochman)

## What This Is

This repository hosts the official landing page for **Found by AI** — a specialized agency service helping NYC (and beyond) local businesses (restaurants, clinics, salons, shops, services) thrive in the AI-first search era.

In 2026, most discovery happens inside conversational AI:  
- Users ask "best NYC sushi for date night" → ChatGPT/Gemini recommends  
- No clicks needed → zero-click decisions drive calls/bookings/revenue

We optimize websites, content, structured data, and agent-ready actions so your business gets **recommended, cited, and acted upon** first — the way Yelp or Google Maps once dominated local search.

## Core Value Proposition

We ensure your business appears where high-intent decisions are made today:  
- Inside AI answers, lists, and recommendations  
- As a trusted source LLMs cite confidently  
- Through direct agent actions (e.g., "book a table" via WebMCP)

Traditional SEO keeps you alive. **Found by AI** makes you dominate the generative age.

## Services Overview

- **AI Readiness Audit** — Test current visibility across major LLMs  
- **AEO & Structured Content** — FAQ/HowTo pages + JSON-LD schema (LocalBusiness, Review, etc.)  
- **GEO / LLMO Authority Building** — Deep content, E-E-A-T signals, multi-platform presence  
- **Technical Implementation** — Custom Schema.org markup for NYC entities  
- **Agentic Readiness (WebMCP)** — Expose actions for AI agents (reserveTable, addToCart)  
- **Ongoing Monitoring** — Track citations & adapt to LLM changes

Targeted for NYC small-medium businesses losing out on AI-driven traffic.

## Landing Page Details

This repo deploys a static landing page via **GitHub Pages**.

### Tech Stack (Current)
- HTML + CSS (Tailwind CSS recommended for rapid styling)  
- JavaScript (vanilla or lightweight like Alpine.js for interactivity)  
- Optional: Next.js / Astro / Vite for static export (if upgrading)

### Deployment
1. Files are in the root (or `/docs` folder if preferred)
2. Go to **Settings > Pages** → Set source to **main** branch → Root
3. Custom domain configured: **discovered-by-ai.com**

### Development
```bash
# Clone
git clone https://github.com/hoomri/discovered-by-ai.git
cd discovered-by-ai

# Edit index.html, styles.css, etc.
# Open locally: just double-click index.html or use live server
